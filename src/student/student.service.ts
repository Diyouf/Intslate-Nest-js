...
export class StudentService {
    constructor(
        ...
        @InjectModel('fee-structure') private readonly feeStructure: Model<FeeStructure>,
                ...
                if (checkId) {
                    if (checkId.password) {
                        ...
                        if (checkId.email !== email) {
                            ...
                            const addpass = await this.studentModel.findOneAndUpdate({ studentId }, { $set: { password: hashedPass } })
                            if (addpass) {
                                return { succes: true }
                            }
                        } else {
            ...
            if (idMatch) {
                if (!idMatch.password) {
                    ...
                    if (!passMatch) {
                        ...
                        const payload = { sub: idMatch._id, email: idMatch.email, role: 'student' };
                        return {
                          ...
                          <table style="max-width:670px;margin:50px auto 10px;background-color:#fff;padding:50px;-webkit-border-radius:3px;-moz-border-radius:3px;border-radius:3px;-webkit-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);-moz-box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24);box-shadow:0 1px 3px rgba(0,0,0,.12),0 1px 2px rgba(0,0,0,.24); border-top: solid 10px green;">
                            ...
                            <tbody>
                              ...
                              <td style="width:50%;padding:20px;vertical-align:top">
                                  <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">ID No.</span>#${studentData.studentId}</p>
                                  <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px">Name</span>${studentData.name}</p>
                                  <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">Email</span> ${studentData.email}</p>
                                  <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">Phone</span> +91-${studentData.phone}</p>
                                </td>
                                <td style="width:50%;padding:20px;vertical-align:top">
                                  <p style="margin:0 0 10px 0;padding:0;font-size:14px;"><span style="display:block;font-weight:bold;font-size:13px;">Address</span> ${studentData.address},${studentData.city} ,${studentData.state}, ${studentData.zip}</p>
                                </td>
                              </tr>
